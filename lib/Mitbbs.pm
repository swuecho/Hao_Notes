...
sub pull_articles {
    ...
    my $collection = $ua->get($full_href)->res->dom->find('a[href^="/article_t/"]');
    #TODO: get sub out
    my @articles = $collection->map(sub { Mitbbs::Article->new(link => $_->{href}, title => decode('gb2312',$_->text)) } )->each;
